---
title: "Events Participated"
date: 2024-01-01
weight: 4
chapter: false
pre: " <b> 4. </b> "
---

During my internship, I participated in 3 events. Each event was a memorable experience that provided valuable real-world engineering insights, opportunities to network with industry leaders, and enhanced Cloud & AI system architecture mindsets.

### [Event 1](4.1-Event1/)
&emsp;**Event Name:** AWS FC Community Day

&emsp;**Date & Time:** 09:00, June 15, 2024

&emsp;**Location:** 26th & 36th Floors, Bitexco Financial Tower, 02 Hai Trieu Street, Ben Nghe Ward, District 1, Ho Chi Minh City (Online)

&emsp;**Role:** Attendee

&emsp;**Brief Description:** A technical deep-dive community event featuring industry experts sharing insights on Agentic Infrastructure Platforms, Vietnamese Voice AI Agents, AWS DevOps Agent for DDoS mitigation, Amazon Q Business for HR automation, and Private VPC Security for MCP Servers.

&emsp;**Outcomes & Value Gained:** Mastered Human-in-the-loop operational mindsets for production-critical systems, Vietnamese Voice AI architecture pipelines (STT-LLM-TTS), and Zero Trust security implementations for private MCP Servers.

---

### [Event 2](4.2-Event2/)
&emsp;**Event Name:** AWS Study Tour & Cloud AI Journey

&emsp;**Date & Time:** 09:00, July 20, 2024

&emsp;**Location:** 26th & 36th Floors, Bitexco Financial Tower, 02 Hai Trieu Street, Ben Nghe Ward, District 1, Ho Chi Minh City

&emsp;**Role:** Attendee

&emsp;**Brief Description:** An interactive office tour and sharing session for students, covering Cloud job market trends from AWS leadership, practical differences in Data Architecture between academia and enterprises, workplace communication/small talk skills, and fresher engineering mindsets in the AI era.

&emsp;**Outcomes & Value Gained:** Understood the Cloud-First recruitment landscape, established long-term consistency and lifelong learning habits, and improved professional workplace communication skills.

---

### [Event 3](4.1-Event3/)
&emsp;**Event Name:** Agentic AI Buildweek Hackathon

&emsp;**Date & Time:** 08:00, August 10, 2024 (24-Hour Hackathon)

&emsp;**Location:** 26th & 36th Floors, Bitexco Financial Tower, 02 Hai Trieu Street, Ben Nghe Ward, District 1, Ho Chi Minh City

&emsp;**Role:** Attendee

&emsp;**Brief Description:** A 24-hour hands-on hackathon focused on building AI Agent applications using AWS Bedrock. The event brought together talented teams building solutions such as KFC Conversational Ordering Agents, Competitor Intelligence Platforms, SA Architecture Generators, Real-Time Crowd Monitoring, and Anti-Money Laundering (AML) Investigation Engines.

&emsp;**Outcomes & Value Gained:** Mastered Multi-Agent architecture patterns (Supervisor - Sub-agents), scope management skills under high time pressure, teamwork collaboration, and technical integration of AWS Bedrock, Step Functions, and Kinesis.
